...
from baserec.base.recommender_utils import similarityMatrixTopK
...
class EASE_R_Recommender(BaseItemSimilarityMatrixRecommender):
    ...
    def fit(self, topK=None, l2_norm=1e3, normalize_matrix=False, verbose=True):
        ...
        similarity = ComputeSimilarity(self.URM_train, shrink=0,
                                       topK=self.URM_train.shape[1], normalize=False, similarity="cosine")
        grahm_matrix = similarity.compute_similarity().toarray()

        diag_indices = np.diag_indices(grahm_matrix.shape[0])

        # The ComputeSimilarity object ensures the diagonal of the similarity matrix is zero

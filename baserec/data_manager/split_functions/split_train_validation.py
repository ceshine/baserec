...
def split_train_validation_percentage_user_wise(URM_train, train_percentage=0.1, verbose=True):
    ...
    if user_no_item_train != 0:
        print("Warning split: {} users with 0 train items ({} total users)".format(
            user_no_item_train, URM_train.shape[0]))
    if user_no_item_validation != 0:
        print("Warning split: {} users with 0 validation items ({} total users)".format(
            user_no_item_validation, URM_train.shape[0]))
...
def split_data_train_validation_test_negative_user_wise(URM_all, negative_items_per_positive=50):
    ...
    for user_index in range(URM_train_all.shape[0]):
        ...
        if num_negative_items > len(unobserved_items):
            print("split_data_train_validation_test_negative: WARNING number of negative to sample for user {} is greater than available negative items {}".format(
...
def split_train_validation_test_negative_leave_one_out_user_wise(URM_all, negative_items_per_positive=50, verbose=True, at_least_n_train_items_test=0, at_least_n_train_items_validation=0):
    ...
    print('Creation test...')
    URM_train_all, URM_test = split_train_validation_leave_one_out_user_wise(
        URM_all, at_least_n_train_items=at_least_n_train_items_test, verbose=verbose)

    print('Creation validation...')
    URM_train, URM_validation = split_train_validation_leave_one_out_user_wise(
        URM_train_all, at_least_n_train_items=at_least_n_train_items_validation, verbose=verbose)
...
def split_train_validation_percentage_random_holdout(URM_train, train_percentage=0.8):
    ...
    URM_validation_builder.add_data_lists(
        URM_train.row[validation_mask], URM_train.col[validation_mask], URM_train.data[validation_mask])

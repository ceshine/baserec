...
from .metrics import (
    ...
    DiversityMeanInterList, DiversityHerfindahl, AveragePopularity
)
...
def _create_empty_metrics_dict(
        cutoff_list, n_items, n_users, URM_train, URM_test, ignore_items,
    ...
    for cutoff in cutoff_list:
        ...
        for metric in EvaluatorMetrics:
            if metric == EvaluatorMetrics.COVERAGE_ITEM:
                ...
                cutoff_dict[metric.value] = GiniDiversity(n_items, ignore_items)

            elif metric == EvaluatorMetrics.SHANNON_ENTROPY:
                cutoff_dict[metric.value] = ShannonEntropy(n_items, ignore_items)
                ...
                cutoff_dict[metric.value] = DiversityMeanInterList(n_items, cutoff)

            elif metric == EvaluatorMetrics.DIVERSITY_HERFINDAHL:
                cutoff_dict[metric.value] = DiversityHerfindahl(n_items, ignore_items)
...
def get_result_string(results_run, n_decimals=7):
    ...
    for cutoff in results_run.keys():
        ...
        for metric in results_run_current_cutoff.keys():
            output_str += "{}: {:.{n_decimals}f}, ".format(metric,
                                                           results_run_current_cutoff[metric], n_decimals=n_decimals)
...
class Evaluator(object):
    ...
    def __init__(self, URM_test_list, cutoff_list, min_ratings_per_user=1, exclude_seen=True,
        ...
        if not np.all(users_to_evaluate_mask):
            self._print(
                "Ignoring {} ({:.2f}%) Users that have less than {} test interactions".format(

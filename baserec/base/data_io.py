...
class DataIO(object):
    ...
    def save_data(self, file_name, data_dict_to_save):
        ...
        for attrib_name, attrib_data in attribute_to_json_file.items():
            ...
            absolute_path = current_file_path + \
                ".json" if current_file_path.startswith(os.getcwd()) else os.getcwd() + current_file_path + ".json"
    ...
    def load_data(self, file_name):
        ...
        try:
            ...
            try:
                attribute_to_file_name_path = dataFile.extract(
                    ".DataIO_attribute_to_file_name.json", path=current_temp_folder)
            except KeyError:
                attribute_to_file_name_path = dataFile.extract(
                    "__DataIO_attribute_to_file_name.json", path=current_temp_folder)
            ...
            for attrib_name, file_name in attribute_to_file_name.items():
                ...
                if attrib_data_type == "csv":
                    ...
                    raise Exception("Attribute type not recognized for: '{}' of class: '{}'".format(
                        attrib_file_path, attrib_data_type))

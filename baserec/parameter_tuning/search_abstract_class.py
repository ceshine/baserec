...
from ..base.data_io import DataIO
from ..base.incremental_training_early_stopping import IncrementalTrainingEarlyStopping
...
class SearchAbstractClass(object):
    ...
    def _set_search_attributes(self, recommender_input_args,
        ...
        if evaluate_on_test not in self._EVALUATE_ON_TEST_VALUES:
            raise ValueError("{}: parameter evaluate_on_test must be in '{}', provided was '{}'.".format(
        ...
        if save_model == "last" and recommender_input_args_last_test is None:
            self._write_log("{}: parameter save_model is 'last' but no recommender_input_args_last_test provided, saving best model on train data alone.".format(
                self.ALGORITHM_NAME))
            save_model = "best"
    ...
    def _objective_function(self, current_fit_parameters_dict):
        ...
        try:
            ...
            if isinstance(recommender_instance, IncrementalTrainingEarlyStopping):
